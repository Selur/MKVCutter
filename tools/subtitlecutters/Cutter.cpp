...
Cutter::Cutter(QObject *parent, bool debug) :
    QObject(parent), m_input(QString()), m_output(QString()), m_acceptedExtensions(), m_debug(debug)
{
}
...
void addNewEntries(const SubtitleEntry &oldEntry, const QList<Cut> &cuts, const QList<Cut> &newCuts, const int &fromIndex, const int &toIndex, const int &fromBefore, const int &toAfter, QList<SubtitleEntry> &adjustedEntries, double &lastEnd, double &previousTo, const bool &debug)
{
    if (debug) {
        cout << " -> fromIndex: " << fromIndex << ", toIndex: " << toIndex << ", fromBefore: " << fromBefore << ", toAfter: " << toAfter << endl;
    }
    SubtitleEntry newEntry = oldEntry;
    if (fromIndex != -1) { // entry inside cut number fromIndex
        if (debug) {
            cout << " newEntry.from = qMax(lastEnd["<<lastEnd<<"], newCuts.at(fromIndex).from["<<newCuts.at(fromIndex).from<<"]) + oldEntry.from["<<oldEntry.from<<"] - qMax(previousTo["<<previousTo<<"], cuts.at(fromIndex).from)["<<cuts.at(fromIndex).from <<"])" << endl;
        }
        newEntry.from = qMax(lastEnd, newCuts.at(fromIndex).from) + oldEntry.from - qMax(previousTo, cuts.at(fromIndex).from);
        if (debug) {
    ...
    if (toIndex != -1) { // entry inside cut number fromIndex
        if (debug) {
            cout << " qMax(newEntry.from["<<newEntry.from<<"], newCuts.at(toIndex).from["<<newCuts.at(toIndex).from<<"]) + oldEntry.to["<<oldEntry.to<<"] -  qMax(oldEntry.from["<<oldEntry.from<<"], cuts.at(toIndex).from["<<cuts.at(toIndex).from<<"])" << endl;
        }
        newEntry.to = qMax(newEntry.from, newCuts.at(toIndex).from) + oldEntry.to -  qMax(oldEntry.from, cuts.at(toIndex).from);
    } else {
        int after = toAfter;
    ...
    if (newEntry.to - newEntry.from < 0.4) {
       if (debug) {
         cout << " -> ignored entry " << newEntry.from << "-" << newEntry.to << " since it's displayed to short" << endl;
       }
       return;
    }
    if (debug) {
...
void setCutIndicesForEntry(int &fromIndex, int &toIndex, int &fromBefore, int &toAfter, const QList<Cut> &cuts, bool &finished, const SubtitleEntry &entry, const bool &debug)
{
    Cut cut;
...
void Cutter::adjustTimingsofEntries(QList<SubtitleEntry> &entries, const QStringList &cutList)
{
    ...
    for (int j = 0, d = entries.count(); j < d; ++j) {
         ...
         if (fromIndex == -1 && toIndex == -1 && (fromBefore >= toAfter || fromBefore == -1)) {
            ...
            addNewEntries(entry, cuts, newCuts, fromIndex, toIndex, fromBefore, toAfter, adjustedEntries, lastEnd, previousTo, m_debug);
         }
         if (finished) {
             break;
         }
...
void outputEntries(const QList<SubtitleEntry> &entries)
{
    ...
    foreach(SubtitleEntry entry, entries) {
        cout << " entry " << entry.from << "-" << entry.to << " = " << qPrintable(entry.text) << ", length: " << entry.to - entry.from << endl;
    }
}
