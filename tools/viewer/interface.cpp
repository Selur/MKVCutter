...
{
  return
      IsPlanar() ? ((pixel_type & CS_PLANAR_MASK) == (c_space & CS_PLANAR_FILTER)) :
...
{
  return
      (fps_numerator && HasVideo()) ?
          ((__int64)(frames) * audio_samples_per_second * fps_denominator / fps_numerator) : 0;
}
int VideoInfo::FramesFromAudioSamples(__int64 samples) const
{
  return
      (fps_denominator && HasAudio()) ?
          (int) ((samples * (__int64 ) fps_numerator)
              / ((__int64 ) fps_denominator * (__int64 ) audio_samples_per_second)) : 0;
}
__int64 VideoInfo::AudioSamplesFromBytes(__int64 bytes) const
{
  return HasAudio() ? bytes / BytesPerAudioSample() : 0;
}
...
{
  return
      !IsY8() && IsPlanar() ? pixels << ((pixel_type >> CS_Shift_Sample_Bits) & 3) :
          pixels * (BitsPerPixel() >> 3);   // For planar images, will return luma plane
}
...
int VideoInfo::RowSize(int plane) const
{
  ...
  switch (plane) {
    ...
    case PLANAR_V_ALIGNED :
      return
          (!IsY8() && IsPlanar()) ?
              ((rowsize >> GetPlaneWidthSubsampling(plane)) + FRAME_ALIGN - 1)
                  & (~(FRAME_ALIGN - 1)) : 0; // Aligned rowsize
...
int VideoInfo::BMPSize() const
{
  if (!IsY8() && IsPlanar()) {
    ...
    const int UVbytes = ((RowSize(PLANAR_U) + 3) & ~3) * height
...
//      return 32;
  ...
  if (IsPlanar()) {
    const int S =
        IsYUV() ? GetPlaneWidthSubsampling(PLANAR_U) + GetPlaneHeightSubsampling(PLANAR_U) : 0;
...
{
  unsigned __int64
  numerator = UInt32x32To64(fps_numerator, multiplier);
  unsigned __int64
  denominator = UInt32x32To64(fps_denominator, divisor);
  ...
  unsigned __int64
  temp = numerator | denominator; // Just looking top bit
  unsigned u = 0;
...
const BYTE* VideoFrameBuffer::GetReadPtr() const
{
  return data;
}
/* Baked ********************
 ...
 Baked ********************/
BYTE* VideoFrameBuffer::GetWritePtr()
{
  InterlockedIncrement(&sequence_number);
  return data;
}
int VideoFrameBuffer::GetDataSize()
{
  return data_size;
}
int VideoFrameBuffer::GetSequenceNumber()
{
  return sequence_number;
}
int VideoFrameBuffer::GetRefcount()
{
  return refcount;
}
...
int __stdcall IClip::GetVersion()
{
...
// class PClip
IClip* PClip::GetPointerWithAddRef() const
{
  if (p)
    p->AddRef();
  return p;
...
void PClip::operator=(IClip* x)
{
  Set(x);
}
void PClip::operator=(const PClip& x)
{
  Set(x.p);
}

IClip* PClip::operator->() const
{
  return p;
}

// useful in conditional expressions
PClip::operator void*() const
{
  return p;
}
bool PClip::operator!() const
{
  return !p;
}
...
PVideoFrame::PVideoFrame(const PVideoFrame& x)
{
  Init(x.p);
}
PVideoFrame::PVideoFrame(VideoFrame* x)
{
  Init(x);
}

void PVideoFrame::operator=(VideoFrame* x)
{
  Set(x);
}
void PVideoFrame::operator=(const PVideoFrame& x)
{
  Set(x.p);
}

VideoFrame* PVideoFrame::operator->() const
{
  return p;
}

// for conditional expressions
PVideoFrame::operator void*() const
{
  return p;
}
bool PVideoFrame::operator!() const
{
  return !p;
}
...
AVSValue::AVSValue(const PClip& c)
{
  type = 'c';
  clip = c.GetPointerWithAddRef();
}
AVSValue::AVSValue(bool b)
{
  type = 'b';
  boolean = b;
}
AVSValue::AVSValue(int i)
{
  type = 'i';
  integer = i;
}
//  AVSValue(__int64 l) { type = 'l'; longlong = l; }
AVSValue::AVSValue(float f)
{
  type = 'f';
  floating_pt = f;
}
AVSValue::AVSValue(double f)
{
  type = 'f';
  floating_pt = float(f);
}
AVSValue::AVSValue(const char* s)
{
  type = 's';
  string = s;
}
/* Baked ********************
 ...
 Baked ********************/
AVSValue::AVSValue(const AVSValue* a, int size)
{
  type = 'a';
  array = a;
  array_size = (short) size;
}
AVSValue::AVSValue(const AVSValue& v)
{
  Assign(&v, true);
}
...
AVSValue& AVSValue::operator=(const AVSValue& v)
{
  Assign(&v, false);
  return *this;
}
...
bool AVSValue::AsBool(bool def) const
{
  _ASSERTE(IsBool()||!Defined());
  return IsBool() ? boolean : def;
}
int AVSValue::AsInt(int def) const
{
...
float AVSValue::AsFloat(float def) const
{
  _ASSERTE(IsFloat()||!Defined());
  return IsInt() ? integer : type == 'f' ? floating_pt : def;
}
const char* AVSValue::AsString(const char* def) const
{
